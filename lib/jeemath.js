...
(function (global) {
  ...
  const SYMBOLS = {
    ...
    boldsymbol:"", vec:"", hat:"", bar:"", tilde:"", dot:"", ddot:"",
  ...
  function readArg(tokens, pos) {
    // Skip whitespace tokens (already skipped in tokeniser)
  ...
  function parseToken(tok, tokens, pos) {
    ...
    if (["vec","hat","bar","tilde","dot","ddot","overline","underline","widehat","widetilde","acute","grave"].includes(cmd)) {
  ...
  function parseEnv(envName, tokens, pos) {
    if (envName === "matrix" || envName === "pmatrix" || envName === "bmatrix" ||
        envName === "vmatrix" || envName === "Vmatrix" || envName === "cases") {
    ...
    while (i < text.length) {
      ...
      for (const d of delimiters) {
        if (text.startsWith(d.open, i)) {
          ...
          if (end !== -1) {
            result += renderMath(text.slice(start, end), d.display);
